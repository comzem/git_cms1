...
$dirType=4;//1:按天存入目录 2:按月存入目录 3:按扩展名存目录  建议使用按天存4:不建立目录
...
if($err==''){
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			...
			$rPath=str_replace('../../', '', $targetPath);
			$rPath=str_replace('//', '/', $rPath);
			if($msgType==1)$msg="'$rPath'";
			else {				
				$msg="{'url':'".$rPath."','localname':'".jsonString($localName)."','id':'1'}";
